...
public class PersonalityEngine {
    ...
    private final Map<String, Float> overriddenTraits = new HashMap<>();
    private final Map<String, Float> unmodifiableTraitView = Collections.unmodifiableMap(this.activeTraits);
    ...
    public Map<String, Float> getOverrides() {
        return Collections.unmodifiableMap(this.overriddenTraits);
    }

    /**
     * Returns an unmodifiable view of the base active traits.
     * If you need the merged result of traits + overrides (e.g. for display), use {@link #getMergedTraits()}.
     */
    public Map<String, Float> getActiveTraits() {
        return this.unmodifiableTraitView;
    }

    /**
     * Returns a merged snapshot of active traits with overrides applied.
     * This allocates a new map — avoid calling per-tick. Use {@link #getTrait(String)} for hot paths.
     */
    public Map<String, Float> getMergedTraits() {
