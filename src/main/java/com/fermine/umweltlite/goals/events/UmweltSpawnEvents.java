...
import com.fermine.umweltlite.api.config.UmweltConfig;
...
/**
 * Handles systemic host hijacking configurations and cognitive initiation routines.
 */
...
public class UmweltSpawnEvents {
    ...
    public static void onEntityJoinLevel(EntityJoinLevelEvent event) {
        ...
        if (!(originalMob instanceof IUmweltEntity)) {
            // Early exit if hijacking config is disabled globally
            if (!UmweltConfig.ENABLE_HOST_HIJACKING.get()) {
                return;
            }

            if (originalMob.getPersistentData().contains(RESET_TAG)) {
                return;
            }
        ...
        if (originalMob instanceof IUmweltEntity umweltMob) {
            if (originalMob.getPersistentData().getBoolean(AWAKENED_TAG)) {
                return;
            }
            ...
            // Flag as awakened immediately to prevent double-processing loops
            originalMob.getPersistentData().putBoolean(AWAKENED_TAG, true);

            AABB area = originalMob.getBoundingBox().inflate(10.0);
            List<Mob> peers = event.getLevel().getEntities(
                    originalMob,
                    area,
                    e -> e.getType() == originalMob.getType() && e != originalMob
            ).stream().filter(Mob.class::isInstance).map(Mob.class::cast).toList();
    ...
    private static void replaceAndCancel(Mob original, EntityType<? extends Mob> targetType, ServerLevel level, EntityJoinLevelEvent event) {
        ...
        // Safe execution processing to avoid threading race conditions during initial chunk generation
        level.getServer().execute(() -> {
            ...
            // Break reference links from original instance
